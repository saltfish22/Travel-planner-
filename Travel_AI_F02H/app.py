...
def load_rerank_model(model_name=rerank_model_name):
    ...
    if os.path.exists(rerank_model_path):
        ...
        try:
            ...
            os.system(f'cd {rerank_path} && git lfs pull') # 已失效网址
...
def embedding_make(text_input, pdf_directory):
    ...
    if len(city_list) != 0:
        ...
        question=text_input
        # 进行初步检索，获得与问题最相关前20个文档片段
        retriever = BM25Retriever.from_documents(splits)
        retriever.k = 20
        bm25_result = retriever.invoke(question)

        # 星火的文本向量模型，为用户的问题和检索到的文档片段生成嵌入向量
        em = EmbeddingModel(config)
        question_vector = em.get_embedding(question) #对问题生成嵌入向量
        pdf_vector_list = []
        ...
        for i in range(len(bm25_result)):
            x = em.get_embedding(bm25_result[i].page_content) # 检索的结果生成嵌入向量
            pdf_vector_list.append(x)
            time.sleep(0.65)

        query_embedding = np.array(question_vector)
        query_embedding = query_embedding.reshape(1, -1)
        # 使用余弦相似度，计算问题与文档片段的相似度
...
def get_location_coordinate(location, city):
    ...
    print(url)
    r = requests.get(url)
    result = r.json() # 解析为json对象
    if "pois" in result and result["pois"]: # 找有没有POI，兴趣点
        return result["pois"][0] # 获得经纬度
    return None
...
def process_request(prompt):
    ...
    while (response.tool_calls is not None):
        ...
        for tool_call in response.tool_calls:
            args = json.loads(tool_call.function.arguments) # 解析json参数
            print(args)
        ...
        response = get_completion(messages) #更新了message，直到能够直接输出而不是调用函数
        if (response.content is None):  # 解决 OpenAI 的一个 400 bug
            response.content = ""
        messages.append(response)  # 把大模型的回复加入到对话中
